...
def remove_contact():
    try:
        global contact_list
        remove_list = []
        for c in remove_list_box.curselection():
            remove_list.append(remove_list_box.get(c))
        if len(remove_list) != 0:
            result_str = ""
            for cont in remove_list:
                for c in contact_list:
                    temp = str(c)
                    if cont == temp:
                        result_str += c["fn"] + " " + c["ln"] + " has been removed from the contact list." + "\n"
                        contact_list.remove(c)
                        break
            messagebox.showinfo("Remove Results", result_str)
    except:
        print("Wrong")
# end remove_contact()
...
def find_remove_contact():
    ...
    if remove_search_value == FIRST_NAME:
        ...
        for c in contact_list:
            if c['eml'] == query:
                results.append(c)
    remove_list_box.delete(0, END)
    for c in results:
        remove_list_box.insert(results.index(c), c)
# end find_remove_contact()
...
remove_search_frame = Frame(remove_container_frame, bg = "cyan")
remove_search_lbl = Label(remove_search_frame, width = 16, anchor = W, text = "Search by:", bg = "cyan")

remove_search_menu = Menubutton(remove_search_frame, textvariable = remove_search_txt, width = 16, bg = "cyan")
...
remove_search_field = Entry(remove_search_frame, width = 32, textvariable = remove_search_str)

remove_search_btn = Button(remove_search_frame, text = "FIND", bg = "green", command = find_remove_contact)
remove_clear_search_btn = Button(remove_search_frame, text = "CLEAR", bg = "red", command = clear_remove_search)

remove_list_box = Listbox(remove_container_frame, bg = "white", selectmode = MULTIPLE, width = 80)
remove_btn = Button(remove_container_frame, bg = "red", text = "REMOVE", command = remove_contact)
